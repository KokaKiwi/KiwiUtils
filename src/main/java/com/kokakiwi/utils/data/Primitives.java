 ...
 * Modified by Koka El Kiwi (http://kokaelkiwi.tk):
 * - toBigInt(byte[] data)
 * - toBytea(BigInteger data)
 */
public class Primitives
{
    ...
    public static byte[] toByta(short[] data)
    {
        ...
        // ----------
        final byte[] byts = new byte[data.length * 2];
    ...
    public static byte[] toByta(BigInteger data)
    {
        if (data == null)
        {
            return null;
    ...
    public static byte[] toByta(String[] data)
    {
        ...
        // ----------
        final byte[] bytes = new byte[totalLength]; // final array
        System.arraycopy(dLen, 0, bytes, 0, 4);
        final byte[] bsLens = toByta(sLens); // byte version of String sLens
        System.arraycopy(bsLens, 0, bytes, 4, bsLens.length);
        // -----
        bytesPos += 4 + bsLens.length; // mark position
        // -----
        for (final byte[] sba : strs)
        {
    ...
    public static boolean[] toBooleanA(byte[] data)
    {
        ...
        // ----------
        final int len = toInt(new byte[] { data[0], data[1], data[2], data[3] });
